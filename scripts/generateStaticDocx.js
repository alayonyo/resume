    ...
    sections: [
      {
        ...
        children: [
          ...
          ...resumeData.professionalSummary.map((paragraph, index) => 
            new Paragraph({
              children: [
                new TextRun({
              ...
              spacing: { 
                after: index < resumeData.professionalSummary.length - 1 ? 200 : 400 
              },
            })
          ),
...
const generateDocx = async () => {
  try {
    ...
    fs.writeFileSync(path.join(publicDir, 'yonatan-ayalon-resume.docx'), buffer);
    console.log('✅ ATS-optimized DOCX generated: /public/yonatan-ayalon-resume.docx');

    console.log('\n🎉 ATS-Optimized Static DOCX generated successfully!');
    console.log('📋 ATS Features:');
    console.log('   • Clean, simple formatting for maximum parsing compatibility');
    ...
    console.log('📄 Content synced from shared data source in /data/resume.json');
  } catch (error) {
